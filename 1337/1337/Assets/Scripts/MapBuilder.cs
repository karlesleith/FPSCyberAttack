...
public class MapBuilder : MonoBehaviour
{
    ...
    void Start()
    {
        mP = GameObject.Find("menuParameters").GetComponent<menuParameters>();

        C = Camera.main.GetComponent<GameCtrl>();
        ...
        if (type == Type.builder)
        {
            ...
            // 
            hex = Resources.Load<GameObject>("wall");
            ...
            // 
            if (SceneManager.GetActiveScene().name == "createdMap") 
            {
    ...
    void GenHex(GameObject F)
    {
        ...
        for (int i = 0; i < size.x; i++)
        {
            if (!data.Contains(id))
            {
                ...
                MapBuilder mb = H.AddComponent<MapBuilder>();
                mb.mat = mat;
                mb.type = Type.block;
                mb.id = id;
                mb.perim = true;
            ...
            for (int j = 1; j < size.y; j++)
            {
                if (!data.Contains(id))
                {
                    if (size.y > 1)
                    {
                        ...
                        MapBuilder mb2 = I.AddComponent<MapBuilder>();
                        mb2.mat = mat;
                        mb2.type = Type.block;
                        mb2.id = id;
    ...
    void Update()
    {
        if (type == Type.builder)
        {
            ...
            if (SceneManager.GetActiveScene().name == "coursebuilder")
            {
                ...
                {
                    MapBuilder MB = hit.collider.GetComponent<MapBuilder>();

                    // 
                    if (Input.GetMouseButton(0) && MB.perim == false)
                    {
                        if (!remove)
                        {
                            if (spent == false)
                        ...
                        else
                        {
                            ...
                            if (selHexx.Count > 0)
                            {
                                MapBuilder MB2 = selHexx[0].GetComponent<MapBuilder>();
                                int ch = MB2.id;

                                //
                                if (ch != stLoc)
                                {
                                    S.transform.position = MB2.transform.position + (Vector3.up * 2.05f);
                    ...
                    else if (Input.GetMouseButton(1))
                    {
                        ...
                        if (selHexx.Count > 0)
                        {
                            MapBuilder LB2 = selHexx[0].GetComponent<MapBuilder>();
                            int ch = LB2.id;
    ...
    public void SaveLvl()
    {
        ...
        {
            saveFile += selHexx[i].GetComponent<MapBuilder>().id + ((i < selHexx.Count - 1) ? "," : string.Empty);
        }
